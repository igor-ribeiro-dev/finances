...
| # | Feature | Description | Status | Depends On |
|---|---------|-------------|--------|------------|
| 012 | Recurring Expenses | Mark an expense as recurring so it is automatically logged at its defined interval without manual re-entry. Supported intervals: monthly and annual. When the scheduled day does not exist in a given month (e.g., the 31st in February), the entry is logged on the last day of that month. A recurring expense can be paused (temporarily skips future periods) or permanently stopped (no future entries generated). Past auto-generated entries are always preserved regardless of pause or stop. Recurring expenses auto-populate the Monthly Payment Tracker (feature 015) as Pending items for each relevant period while active. | Planned | 005 |
